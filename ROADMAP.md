# Roadmap

Planned improvements and future skill ideas. Not a commitment — just a running list.

---

## Query Layer Skills

| Skill | Status | Notes |
|---|---|---|
| `sqlite-query` | ✅ Shipped | Local SQLite index via Python standard library. Zero infrastructure. |
| `graphiti-query` | 📋 Planned | Graph layer using [Graphiti by Zep](https://github.com/getzep/graphiti). Temporal knowledge graph with relationship context. Requires Python + Graphiti install. |

---

## Other Ideas

- **`!! export`** — export the wiki to a portable format (HTML, PDF, etc.) for sharing
- **Vector search skill** — semantic similarity search across pages using local embeddings

---

*Updated: 2026-04-19*
