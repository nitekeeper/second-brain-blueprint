...
B5. Process each file in sequence using **`[main-steps 5, 6, 7, 8, 9, 10, 11]`** per file — `[main-step 2]` was already executed at batch level in B3.5, so do NOT re-read sources here; also skip `[main-steps 1, 3, 4, 12, 13]`, which are handled at batch level (B3, B4, B6). **For the first file only,** `[main-step 8]` uses the cached `index.md` from B3. **For every subsequent file,** re-read `wiki/index.md` fresh at `[main-step 8]` — the previous file's `[main-step 10]` mutated it, and reusing the stale cache causes duplicate page creation and missed cross-links. The log.md tail stays cached across the batch; only index.md needs the per-file refresh.
