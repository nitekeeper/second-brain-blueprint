...
> **Self-cost note:** This file itself is ~2,120 tokens to read. Every approval request requires reading it unless the relevant numbers are already cached in working memory from earlier in the same operation. Include the ~2,120-token cost in quoted estimates for the first approval of an operation; subsequent approvals within the same op can cache.
...
**Recalibration trigger:** Fire immediately when any file's measured actual exceeds its documented Chars value — the headroom has been fully consumed. Also fire pre-emptively when any file's remaining headroom drops below ~10% of its measured actual, so the trigger catches drift before it hard-fires mid-op (this codifies the pattern caught by audits #5 S2, #7 W1, #8 S1, and #9 S1). Also recalibrate after every INGEST operation as a routine pass.
