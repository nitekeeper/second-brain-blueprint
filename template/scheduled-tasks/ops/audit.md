...
- For `!! audit all`, expect ~30,000–45,000 tokens of reads (envelope widened 2026-04-18 in v2.0.6 to contain the documented Tokens-column sum from `token-reference.md` — the source of truth. Re-derive by summing the blueprint-doc and template-side rows rather than hand-tuning this figure; the ~45,000 upper bound is the documented sum plus a small cushion, matching the 110% per-file headroom convention. Future envelope drift should be caught by re-summing the table, not by editing this literal). Warn the user up front if the session is already close to context limits.
