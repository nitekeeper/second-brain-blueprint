...
1. **Read the wiki's stored hashes.** Build a URL → source-page map **once** per run by enumerating `wiki/pages/sources/*.md` and reading each file's `source_url:` frontmatter. Then, for each monitored source:
   ...
   - If **more than one** source page matches, record the source as AMBIGUOUS (new status, added in v2.0.10) and skip the hash lookup — this indicates a duplicate-ingest situation that `!! lint` should surface; the monitor's job is to report the anomaly, not to pick a winner.
   - Pages with `source_url: unknown` (the pre-v2.0.10 fallback value or a missing field) are **not** candidates for matching and contribute nothing to the map — they were either written before the `source_url:` invariant landed or failed to recover a URL from the Clipper preamble. See `troubleshooting.md` "Changelog monitor reports 🆘 UNINGESTED for a source I know I ingested" for the retroactive-fix flow.

   This approach replaces the pre-v2.0.10 "slug-matched, reuse `ops/ingest.md` Step 0 slug derivation rules" instruction, which was literally unexecutable here because Step 1 runs **before** Step 2's fetch and has no H1 or page-title input — only the URL and the Source Title column. Reverse-lookup by `source_url:` is exact, inputs are available pre-fetch, and it no longer depends on ingest-time slug derivation matching anything the monitor can compute.
...
4. **Classify each source:**
   ...
   - **UNINGESTED** — no source page matches the URL via the Step 1 reverse-lookup (either never ingested, or ingested pre-v2.0.10 with `source_url: unknown` / missing — in either case the fix is to run `!! ingest <URL>` and the post-ingest page will carry the canonical URL).
   - **AMBIGUOUS** — more than one source page carries this URL (should not occur absent wiki corruption or duplicate-ingest bugs; reporting here is the monitor's contribution to detection).
...
⚠️ Example Ambiguous Source — AMBIGUOUS (matched N source pages)
...
| ⚠️ | AMBIGUOUS | Multiple source pages carry this URL; run `!! lint` to surface the duplicate |
...
**Trailing hint lines.** Emit `🆕 items: run \`!! ingest <URL>\` to pull the update` only if the message contains at least one 🆕 row; emit `🆘 items: run \`!! ingest <URL>\` to bootstrap` only if it contains at least one 🆘 row. When the message contains only ✅, ❌, and/or ⚠️ rows, omit both trailing hint lines — there is no ingest-style action for the user to take on those statuses. ⚠️ rows carry their own inline action hint (`run \`!! lint\``) in the emoji legend, so no separate trailing hint is needed.
